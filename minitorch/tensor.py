...
class Tensor:
    ...
    def all(self, dim: Optional[Tensor] = None) -> Tensor:
        if dim is None:
            ...
            return All.apply(self, self._ensure_tensor(dim if dim is not None else 0))
    ...
    def sum(self, dim: Optional[TensorLike] = None) -> Tensor:
        if dim is None:
            return Sum.apply(self.contiguous().view(self.size), self._ensure_tensor(0)) # must call contiguous before view
        else:
